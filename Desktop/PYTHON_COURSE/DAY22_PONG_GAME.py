from turtle import Turtle,Screen
from DAY22_Paddle_Class import Padle
from DAY22_Ball_Class import Ball
from DAY22_Scoreboard_Class import Scoreboard
import turtle,time

tim=Turtle()

screen=Screen()
screen.setup(width=800,height=600)
screen.bgcolor('black')
screen.title('Pong')
screen.tracer(0)

r_paddle=Padle((350,0))

l_paddle=Padle((-350,0))

ball=Ball()
r_paddle.speed('fastest')
l_paddle.speed('fastest')

score=Scoreboard()

screen.listen()
screen.onkey(r_paddle.go_up,"Up")
screen.onkey(r_paddle.go_down,"Down")

screen.onkey(l_paddle.go_up,"w")
screen.onkey(l_paddle.go_down,"s")


game_is_on=True
while game_is_on:
    time.sleep(ball.move_speed)
    screen.update()
    ball.move()
    
    #detecting colllision of ball with wall
    if ball.ycor()>280 or ball.ycor()<-280:
        #bounce
        ball.bounce_y()
        
    #detect collision with  paddles
    if ball.distance(r_paddle)<70 and ball.xcor()>320 or ball.distance(l_paddle)<70 and ball.xcor()<-320:
            
            ball.bounce_x()
    
    
    #detected when right paddle misses
    if ball.xcor()>380:
        ball.reset_position()
        score.l_point()
    
    #detect when left paddle misses
    if ball.xcor()<-380:
        ball.reset_position()
        score.r_point()
        
screen.exitonclick()